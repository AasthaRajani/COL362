...
public class PrimaryTestCases {
    ...
    public List<Object []> eval(RelNode relNode, MyCalciteConnection calciteConnection) {
        try{
            ...
            System.out.println("phyRelNode "+phyRelNode);
...
//            String query = "select DISTINCT film.length, count(film.length) from\n"
//                    + "film where rating = 'PG' and length<50 group by film.length";
            ...
            for(Object [] row : result){
                assert(row.length == 1);
//                if(row==null) {
//                    System.out.println("Null row in answer");
//                    break;
//                }
//                for(Object row_i:row) {
//                    System.out.print(row_i + " ");
//                }
//                System.out.println();
            }
            // sort the result
...
//                System.out.println("My value is " + result.get(i)[0] + " Expected val is " + expected[i] + " at index " + i);
    ...
    public void testJoinAggregate_1() {
        try{
            ...
            RelNode relNode = createRelNode(query, calciteConnection);
            List<Object []> result = eval(relNode, calciteConnection);
            System.out.println("Size of result in ja is "+result.size());
            assert(result.size() == 1);
            System.out.println("All three assertions passed 0");
            assert(result.get(0).length == 1);
            System.out.println("All three assertions passed 1 "+ result.get(0)[0]);
            assert(result.get(0)[0].equals(14596));
            System.out.println("All three assertions passed 2");
            calciteConnection.close();
        }
        catch(Exception e){
            System.out.println(e);
